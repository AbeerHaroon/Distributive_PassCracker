...
class GuessGen: 
    ...
    def __init__(self):
        ...
        print(self.flow_signs) #set the flow indicators. 1 by default
    ...
    def crackCycle(self, target_hash):
        ...
        while (length < 21): #20 character guesses
            while (found == 0) or (count == 94^(length)): #if found or 96^(length) times tried
    ...
    def tick(self,g):
        ...
        iterated_str = "" #new string which we will return
        i = 0 #this helps to check the flow in each character

        while (i != len(original_str)): #while i is less than length of guess string
            if i == 0: #if first letter
                if flow_copy[i] == 0: #if decrementing flow
                    if ord(original_str[i]) == 32: 
                        #new_char = chr(33) #incremented. changed flow
                        #g[i:(i+1)] = new_char
